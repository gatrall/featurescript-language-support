...
FeatureScript Language Support adds syntax highlighting, semantic highlighting, snippets, document symbols, folding support, current-file Go to Definition / Find References, hover tooltips, and deterministic enum/map completions for Onshape FeatureScript in VS Code.
...
3. Download the file ending in `.vsix`, for example:
   [featurescript-language-support-0.1.6.vsix](https://github.com/gatrall/featurescript-language-support/releases/latest/download/featurescript-language-support-0.1.6.vsix)
4. Open VS Code.
...
```sh
code --install-extension featurescript-language-support-0.1.6.vsix
```
...
- Hover tooltips for local declaration doc comments, generated standard-library signatures, enum variants, and known feature definition fields.
- Deterministic completions for enum members after `EnumName.` and for known feature definition-map keys such as `extrude(context, id, { ... })`.
- Generated standard-library symbol and metadata indexes committed in the extension; no network access is required at runtime.
...
The generated standard-library indexes are committed at `src/generated/stdlibSymbols.json` and `src/generated/stdlibMetadata.json` and are used at extension runtime without network access.
...
The metadata generator extracts enum variants plus feature definition fields from stdlib docblocks, precondition declarations like `definition.width is number`, validator calls like `isLength(definition.depth, ...)`, and bounded predicate calls such as `booleanStepTypePredicate(definition)`.
...
- Hovers and completions use generated stdlib metadata, but full rendered FsDoc prose is not bundled yet.
- Completion support is intentionally focused on enum members and known feature definition-map keys; it is not broad all-symbol autocomplete.
